...
async def handle_file(message: types.Message, state: FSMContext):
    ...
    await bot.send_document(ADMIN_ID, message.document.file_id, 
                           caption=f"📩 নতুন ফাইল জমা পড়েছে!\n👤 ইউজার আইডি: `{message.from_user.id}`", 
                           reply_markup=keyboard, parse_mode="Markdown")
    
    await message.answer("✅ আপনার ফাইলটি জমা হয়েছে। এডমিন চেক করে ব্যালেন্স দিয়ে দিবে।", reply_markup=main_menu())
    await state.finish()

# ==========================================
# ৩. উইথড্র লজিক (Withdrawal)
# ==========================================
...
async def save_address(message: types.Message, state: FSMContext):
    ...
    db.commit()
    await message.answer("✅ পেমেন্ট মেথড সেভ হয়েছে। আবার 'Withdraw' বাটনে ক্লিক করুন।", reply_markup=main_menu())
    await state.finish()
...
async def withdraw_done(message: types.Message, state: FSMContext):
    try:
        ...
        if amount > balance:
            ...
            db.commit()
            await bot.send_message(ADMIN_ID, f"🔔 উইথড্র রিকোয়েস্ট!\n🆔 আইডি: `{message.from_user.id}`\n💵 পরিমাণ: {amount} ৳\n📍 এড্রেস: {address}")
            await message.answer(f"✅ উইথড্র সফল! {amount} ৳ কেটে নেওয়া হয়েছে।\nবর্তমান ব্যালেন্স: {new_balance} ৳", reply_markup=main_menu())
        await state.finish()
    except: await message.answer("❌ শুধু সংখ্যা লিখুন।")
...
async def admin_check(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        else: await message.answer("❌ ইউজার পাওয়া যায়নি।")
...
async def admin_edit(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        try:
            ...
            db.commit()
            await message.answer(f"✅ ইউজার {args[0]} এর ব্যালেন্স এডিট করা হয়েছে।")
...
async def admin_broadcast(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        await message.answer("✅ সবার কাছে মেসেজ পাঠানো হয়েছে।")
...
async def final_add_money(message: types.Message, state: FSMContext):
    if message.from_user.id == ADMIN_ID:
        try:
            ...
            await message.answer(f"✅ {amount} ৳ সফলভাবে যোগ করা হয়েছে।")
        except: await message.answer("❌ ভুল ইনপুট।")
        await state.finish()

# ==========================================
# ৫. রান করা (সংশোধিত অংশ)
# ==========================================
